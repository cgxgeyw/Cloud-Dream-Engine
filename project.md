...
- 移动聊天流放叙事、角色/玩家消息，以及世界主控思维链和 NPC 思维链；调试、retry 管理卡、角色创建/切换卡不要进入普通移动聊天流。
- 思维链（世界主控 + NPC）以折叠形式进入聊天流，桌面和移动都生效：思维链始终只展示前 200 字，超出部分通过“展开/收起”查看；NPC 正文不折叠。组件用独立 class（`game-cot*`），不复用旧的 `.game-agent-reasoning`（移动 seed 仍 `display:none` 隐藏它）。
- 旁白由前端 `MessageList` 从 agent 消息 `metadata.narration` 渲染为一条独立消息，跟在对应 NPC 消息后；不改后端，不持久化为额外消息。
...
The world list page owns a creator-facing AI flow for drafting worlds. The frontend calls `createWorldWithAi()` from `frontend/src/data/apiAdapter.ts`; in Tauri runtime this invokes `create_world_with_ai`.

Backend flow:
- Command: `src-tauri/src/commands/worlds.rs#create_world_with_ai`.
- Draft generation: `src-tauri/src/services/world_builder.rs`, using the current default text model or the first text model.
- Persistence: generated JSON is normalized into `WorldCreateRequest` and `CharacterCreateRequest`, then saved through the existing world service and character repository.

Single-agent mode creates one character and uses `director_config.service_mode = "agent_chat"` with `default_agent_id` set to that character. Multi-agent mode creates up to five opening characters and uses `director_config.service_mode = "world_sim"`. This feature should stay shared creator tooling, not a built-in-world special case.
